...
ArduboyGraphics::ArduboyGraphics() {
  frameRate = 60;
  frameCount = 0;
...
void ArduboyGraphics::setFrameRate(uint8_t rate)
{
  frameRate = rate;
  eachFrameMillis = 1000/rate;
}

bool ArduboyGraphics::everyXFrames(uint8_t frames)
{
  return frameCount % frames == 0;
}

bool ArduboyGraphics::nextFrame()
{
  long now = millis();
  uint8_t remaining;
...
// events
void ArduboyGraphics::initRandomSeed()
{
...
uint16_t ArduboyGraphics::rawADC(byte adc_bits)
{
  ADMUX = adc_bits;
...
void ArduboyGraphics::clearDisplay()
{
  // C version:
...
void ArduboyGraphics::drawPixel(int x, int y, uint8_t color)
{
...
uint8_t ArduboyGraphics::getPixel(uint8_t x, uint8_t y)
{
  uint8_t row = y / 8;
...
void ArduboyGraphics::drawCircle(int16_t x0, int16_t y0, int16_t r, uint8_t color)
{
  int16_t f = 1 - r;
  int16_t ddF_x = 1;
  int16_t ddF_y = -2 * r;
...
void ArduboyGraphics::drawCircleHelper
...
void ArduboyGraphics::fillCircle(int16_t x0, int16_t y0, int16_t r, uint8_t color)
{
  drawFastVLine(x0, y0-r, 2*r+1, color);
  fillCircleHelper(x0, y0, r, 3, 0, color);
}

void ArduboyGraphics::fillCircleHelper
(
 int16_t x0,
 int16_t y0,
 int16_t r,
...
void ArduboyGraphics::drawFastVLine
...
void ArduboyGraphics::drawFastHLine
...
void ArduboyGraphics::fillScreen(uint8_t color)
{
  fillRect(0, 0, WIDTH, HEIGHT, color);
}

void ArduboyGraphics::drawRoundRect
...
void ArduboyGraphics::fillRoundRect
(int16_t x, int16_t y, int16_t w, int16_t h, int16_t r, uint8_t color)
{
  // smarter version
...
void ArduboyGraphics::drawTriangle
...
void ArduboyGraphics::fillTriangle
(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint8_t color)
{
...
void ArduboyGraphics::drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint8_t color) {
...
void ArduboyGraphics::drawSlowXYBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint8_t color) {
  // no need to dar at all of we're offscreen
  if (x+w < 0 || x > WIDTH-1 || y+h < 0 || y > HEIGHT-1)
    return;
...
void ArduboyGraphics::display()
{
  this->paintScreen(sBuffer);
Serial.write(sBuffer, 1024);
}

unsigned char* ArduboyGraphics::getBuffer(){
  return sBuffer;
}
...
boolean ArduboyGraphics::pressed(uint8_t buttons)
{
...
boolean ArduboyGraphics::not_pressed(uint8_t buttons)
{
 uint8_t button_state = getInput();
 return (button_state & buttons) == 0;
}

void ArduboyGraphics::swap(int16_t& a, int16_t& b) {
  int temp = a;
  a = b;
  b = temp;
}
